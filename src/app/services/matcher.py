...
class SingleMatchResponse(BaseModel):
    ...
    matched_source_index: int | None = Field(None, description="一致した候補のインデックス番号 (0-indexed)。見つからなかった場合は None。")
    confidence: float = Field(description="照合の信頼度スコア (0.0〜1.0)。数値・単位・部位が完全に一致すれば0.95以上。単位換算（例: N/m² と kN/m²）で一致すれば0.85〜0.94。数値が不一致や確証がない場合は0.80未満。")
    ai_reasoning: str = Field(description="一致・不一致の判定理由。単位換算を行った場合はその計算過程を記述してください。")
...
def calculate_candidate_score(check_item: CheckItem, item: SourceItem) -> float:
    ...
    context_sim = token_overlap(cast(str, check_item.context), cast(str, item.context_text))
    ...
    if check_item.source_hint:
        ...
        hint_words = [w for w in re.findall(r'[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\w]+', hint_lower) if len(w) >= 2]
        for hw in hint_words:
            if hw in cast(str, item.context_text).lower() or hw in cast(str, item.label).lower():
                hint_boost += 1.0
                break
...
async def match_check_item(
    ...
    return MatchResult(
        ...
        status="approved" if (response.matched and response.confidence >= 0.90) else "pending",
