...
@router.post(
    "/api/v1/sessions",
    response_model=CheckSessionResponse,
...
async def create_session(
    ...
    try:
        extracted_items = await extract_check_items_from_pdf(
    ...
    return CheckSessionResponse(
        ...
        prompt_template_id=str(session.prompt_template_id)
        if session.prompt_template_id
        else None,
    ...
    for s in sessions:
        ...
        results.append(
            CheckSessionResponse(
                ...
                prompt_template_id=str(s.prompt_template_id)
...
@router.get(
    "/api/v1/sessions/{session_id}/items", response_model=list[CheckItemResponse]
)
...
@router.get(
    "/api/v1/sessions/{session_id}/results", response_model=list[MatchResultResponse]
)
def get_session_results(session_id: str, db: Session = Depends(get_db)):
    ...
    for result in match_results:
        check_item = (
            db.query(CheckItem).filter(CheckItem.id == result.check_item_id).first()
        )
        if not check_item:
            continue
...
def update_result_status(
    result_id: str, payload: ResultStatusUpdate, db: Session = Depends(get_db)
):
    ...
    if result.source_item_id:
        s_item = (
            db.query(SourceItem).filter(SourceItem.id == result.source_item_id).first()
        )
        if s_item:
            source_item = SourceItemInfo(
                label=str(s_item.label),
...
def export_annotated_pdf(session_id: str, db: Session = Depends(get_db)):
    ...
    return FileResponse(
        path=output_path, media_type="application/pdf", filename="annotated_file_a.pdf"
    )
...
def export_excel(session_id: str, db: Session = Depends(get_db)):
    ...
    original_path = Path(cast(str, session.file_a_path))
